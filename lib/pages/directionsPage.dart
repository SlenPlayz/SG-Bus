  ...
  Future<void> getStartingInfo() async {
    ...
    if (startingData?["ul"] != null && startingData?["ul"] == true) {
      getLocation().then((location) {
        ...
        // Handle Map error type which might happen if Future.error is not a map
        // But here we throw map
        showDialog(
            context: context,
            builder: (context) => AlertDialog(
                  icon: Icon(Icons.warning),
                  title: Text(err is Map
                      ? (err["msg"] ?? err.toString())
                      : err.toString()),
                  actions: [
                    (err is Map && err["code"] == 2)
                        ? TextButton(
                            onPressed: () async {
                              await Geolocator.requestPermission();
                              Navigator.of(context).pop();
                              getStartingInfo(); // Retry
                            },
                            child: const Text('Request permission'))
                        : (err is Map &&
                                err["code"] ==
                                    3) // Code 3 was denied forever (settings)
                            ? TextButton(
                                onPressed: () async {
...
Future<List> _getRoutes(Object? ptMode, LatLng startingCoords,
  ...
  try {
    ...
    final url = Uri.parse(
        'https://www.onemap.gov.sg/api/public/routingsvc/route?start=${startingCoords!.latitude},${startingCoords!.longitude}&end=${destCoords!.latitude},${destCoords!.longitude}&routeType=pt&date=${date}&time=${departureTime.hour.toString().padLeft(2, '0')}:${departureTime.minute.toString().padLeft(2, '0')}:00&mode=${ptMode}');
