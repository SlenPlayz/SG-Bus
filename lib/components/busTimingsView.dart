...
class _BusTimingsViewState extends State<BusTimingsView> {
  ...
  Future<void> getArrTimings() async {
    ...
    try {
      ...
      // Don't show dialog in this view as it might be small, just show error state or retry
      // showing dialog might block the entire navigation view
    }
  }
    ...
    return Column(
      children: [
        ...
        Skeletonizer(
          enabled: isLoading,
          child: Column(
            children: [for (var x in arrTimings) BusTiming(x)],
          ),
        )
      ],
    );
